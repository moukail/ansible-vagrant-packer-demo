variable "iso_url" {
  type    = string
  default = "https://repo.almalinux.org/almalinux/9/isos/x86_64/AlmaLinux-9.7-x86_64-boot.iso"
}
...
source "qemu" "almalinux9_vagrant_libvirt_x86_64" {
  ...
  boot_command = [
    ...
    "inst.text inst.ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/almalinux9_ks.cfg",
    "<enter><wait>"
  ]
...
build {
  ...
  post-processors {
    post-processor "vagrant" {
      compression_level = "9"
      output = "./boxes/almaLinux9-vagrant-{{.Provider}}.box"
    }

    post-processor "vagrant-registry" {
      box_tag = "imoukafih/almalinux9"
      version = "1.0.0"
